...
* Search support: the [frontend](https://github.com/source-academy/frontend)'s Text Search and Index Search buttons fetch [json/rewritedSearchData.json](https://sicp.sourceacademy.org/json/rewritedSearchData.json) from this public static host (not the Source Academy backend) and filter it locally; generated by `yarn json` (`javascript/searchRewrite.ts`), which runs as part of `deploy-pages.yml` on every push to `master`
...
* Search support: same mechanism as SICP JS above, via [json_py/rewritedSearchData.json](https://sicp.sourceacademy.org/json_py/rewritedSearchData.json)
...
* Structured per-section JSON textbook content, e.g. [json_scm/1.1.1.json](https://sicp.sourceacademy.org/json_scm/1.1.1.json), including generated search data ([json_scm/rewritedSearchData.json](https://sicp.sourceacademy.org/json_scm/rewritedSearchData.json)) — not yet wired up to a live interactive frontend, unlike JS/Python above
