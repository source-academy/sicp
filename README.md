...
[`chatbot_notes_py/`](chatbot_notes_py) is a hand-curated knowledge base for retrieval-augmented-generation (RAG) Q&A bots over SICPy, published alongside the other editions at [chatbot_notes_py](http://sicp.sourceacademy.org/chatbot_notes_py). Unlike the editions above, it is not generated by the XML build pipeline — it's checked into git directly and only regenerated by hand for major revisions of the book.
