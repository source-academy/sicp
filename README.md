...
* [PDF edition](https://sicp.sourceacademy.org/sicpjs.pdf)
* [Interactive SICP JS](https://sourceacademy.org/sicpjs), backed by structured per-section JSON textbook content, e.g. [json/1.1.1.json](https://sicp.sourceacademy.org/json/1.1.1.json)
* [Comparison edition](https://sicp.sourceacademy.org), Scheme vs. JavaScript
* Search support: the [frontend](https://github.com/source-academy/frontend)'s Text Search and Index Search buttons fetch [json/searchData.json](https://sicp.sourceacademy.org/json/searchData.json) from this public static host (not the Source Academy backend) and filter it locally; generated by `yarn json` (`javascript/search.ts`), which runs as part of `deploy-pages.yml` on every push to `master`
* Table of contents and prev/next navigation: the frontend fetches [json/toc.json](https://sicp.sourceacademy.org/json/toc.json) and [json/toc-navigation.json](https://sicp.sourceacademy.org/json/toc-navigation.json) at runtime, the same way it fetches section content and search data above — it must not snapshot these into its own repo, since the published chapter range can shrink or grow (e.g. `SICP_PUBLISHED_CHAPTERS`) independently of a frontend release; generated by `yarn json`/`yarn split` (`javascript/generateTocJson.js`)
* Source code archive ([sicpjs.zip](https://sicp.sourceacademy.org/sicpjs.zip)), the `programs_js` extracted from the book, one file per snippet
* Single-file Markdown export ([sicpjs.md](https://sicp.sourceacademy.org/sicpjs.md)), mirroring the PDF's content selection, for reading or feeding to tools that don't handle HTML/PDF/XML

### SICPy

* [PDF edition](https://sicp.sourceacademy.org/sicpy.pdf)
...
* Table of contents and prev/next navigation: same mechanism as SICP JS above, via [json_py/toc.json](https://sicp.sourceacademy.org/json_py/toc.json) and [json_py/toc-navigation.json](https://sicp.sourceacademy.org/json_py/toc-navigation.json)
...
* [PDF edition](https://sicp.sourceacademy.org/sicp.pdf)
* Structured per-section JSON textbook content, e.g. [json_scm/1.1.1.json](https://sicp.sourceacademy.org/json_scm/1.1.1.json), including generated search data ([json_scm/searchData.json](https://sicp.sourceacademy.org/json_scm/searchData.json)) — not yet wired up to a live interactive frontend, unlike JS/Python above
* Source code archive ([sicp.zip](https://sicp.sourceacademy.org/sicp.zip)), the `programs_scm` extracted from the book, one file per snippet
* Single-file Markdown export ([sicp.md](https://sicp.sourceacademy.org/sicp.md)), mirroring the PDF's content selection
...
[`chatbot_notes_py/`](chatbot_notes_py) is a knowledge base for retrieval-augmented-generation (RAG) Q&A bots over SICPy, published alongside the other editions at [chatbot_notes_py](https://sicp.sourceacademy.org/chatbot_notes_py) (an `index.html` there lists and links every file). Unlike the editions above, it is not autogenerated by the XML build pipeline: it's AI-generated on demand — not by any pipeline in this repo — and checked into git directly, regenerated only for major revisions of the book.
