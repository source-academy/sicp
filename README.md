...
* JSON edition, index and search data for [Interactive SICPy](https://sourceacademy.org/sicpy) ([json_py](http://sicp.sourceacademy.org/json_py/toc.json)). The [frontend](https://github.com/source-academy/frontend)'s Text Search and Index Search buttons both fetch [`json_py/rewritedSearchData.json`](http://sicp.sourceacademy.org/json_py/rewritedSearchData.json) from this public static host (not from the Source Academy backend) and filter it locally; the table of contents (`toc-py.json`) is instead bundled into the frontend at build time. `json_py` is generated by `yarn json` (`javascript/searchRewrite.ts`), which runs as part of `deploy-pages.yml` on every push to `master`.
...
[`chatbot_notes_py/`](chatbot_notes_py) is a hand-curated knowledge base for retrieval-augmented-generation (RAG) Q&A bots over SICPy, published alongside the other editions at [chatbot_notes_py](http://sicp.sourceacademy.org/chatbot_notes_py) (an `index.html` there lists and links every file). Unlike the editions above, it is not generated by the XML build pipeline — it's checked into git directly and only regenerated by hand for major revisions of the book.
